...
// Section 1: ZOOM MS SERIES Initialization
// Section 2: ZOOM MS SERIES common MIDI in functions
// Section 3: ZOOM MS SERIES common MIDI out functions
// Section 4: ZOOM MS SERIES program change
// Section 5: ZOOM MS SERIES parameter control

// ********************************* Section 1: ZOOM MS SERIES initialization ********************************************

// Zoom MS70CDR settings:
#define ZMS_MIDI_CHANNEL 1
#define ZMS_MIDI_PORT USBHMIDI_PORT
#define ZMS_PATCH_MIN 0
#define ZMS_PATCH_MAX 49
#define ZMS_MIDI_TIMER_LENGTH 100 // Minimal time inbetween requests for patch data from MS70

// Documentation of Zoom sysex has been moved to http://www.vguitarforums.com/smf/index.php?topic=4329.msg131444#msg131444 (The ZOOM MS SERIES v2 MIDI specification).
// More info can be found at https://github.com/g200kg/zoom-ms-utility/blob/master/midimessage.md
...
#define ZMS50G_MODEL_NUMBER 0x58
#define ZMS60B_MODEL_NUMBER 0x5f
#define ZMS70CDR_MODEL_NUMBER 0x61
// 2) The editor keeps sending F0 52 00 61 50 F7. The MS SERIES does not seem to respond to it. But it may signal editor mode on.
#define ZMS_EDITOR_MODE_ON 0x50
// 3) If the editor sends F0 52 00 61 33 F7, the MS SERIES responds with the current Bank number (CC00 and CC32) and the current Program number (PC)
#define ZMS_REQUEST_CURRENT_PATCH_NUMBER 0x33
// 4) If the editor sends F0 52 00 61 29 F7, the MS SERIES responds with the current patch in 110 bytes with command number 28. Byte 0x61 - 0x6B contain the patch name. with a strange 0 at position 0x65
#define ZMS_REQUEST_CURRENT_PATCH 0x29
// 5) The editor then reads all individual patches by sending F0 52 00 61 09 00 00 {00-63} (patch number) F7.
//    The MS SERIES responds with 156 bytes with command number 08. Byte 0x66 - 0x70 contain the patch name. with an "overflow byte" at position 0x6A.
// 6) At the end the editor sends F0 52 00 61 51 F7 and communication seems to stop.
#define ZMS_EDITOR_MODE_OFF 0x51
...
//    Same message is sent for changing a knob on the MS SERIES, but byte 7 is not 0x00
...
FLASHMEM void MD_ZMS_class::init() // Default values for variables
{
  MD_base_class::init();

  // Variables
  enabled = DEVICE_DETECT; // Default value
  strcpy(device_name, "ZMS"); // This line crashes the VController Production Model for no apparent reason - memory leak somewhere? Fixed by changing "Optimize" to fast
  strcpy(full_device_name, "Zoom MS series");
  patch_min = ZMS_PATCH_MIN;
  patch_max = ZMS_PATCH_MAX;
  ...
  MIDI_channel = ZMS_MIDI_CHANNEL; // Default value
  MIDI_port_manual = MIDI_port_number(ZMS_MIDI_PORT); // Default value
#if defined(CONFIG_VCTOUCH)
  my_device_page1 = ZMS_DEFAULT_VCTOUCH_PAGE1; // Default value
  my_device_page2 = ZMS_DEFAULT_VCTOUCH_PAGE2; // Default value
  my_device_page3 = ZMS_DEFAULT_VCTOUCH_PAGE3; // Default value
  my_device_page4 = ZMS_DEFAULT_VCTOUCH_PAGE4; // Default value
#elif defined(CONFIG_VCMINI)
  my_device_page1 = ZMS_DEFAULT_VCMINI_PAGE1; // Default value
  my_device_page2 = ZMS_DEFAULT_VCMINI_PAGE2; // Default value
  my_device_page3 = ZMS_DEFAULT_VCMINI_PAGE3; // Default value
  my_device_page4 = ZMS_DEFAULT_VCMINI_PAGE4; // Default value
#elif defined (CONFIG_CUSTOM)
  my_device_page1 = ZMS_DEFAULT_CUSTOM_PAGE1; // Default value
  my_device_page2 = ZMS_DEFAULT_CUSTOM_PAGE2; // Default value
  my_device_page3 = ZMS_DEFAULT_CUSTOM_PAGE3; // Default value
  my_device_page4 = ZMS_DEFAULT_CUSTOM_PAGE4; // Default value
#else
  my_device_page1 = ZMS_DEFAULT_VC_PAGE1; // Default value
  my_device_page2 = ZMS_DEFAULT_VC_PAGE2; // Default value
  my_device_page3 = ZMS_DEFAULT_VC_PAGE3; // Default value
  my_device_page4 = ZMS_DEFAULT_VC_PAGE4; // Default value
#endif
...
FLASHMEM void MD_ZMS_class::update() {
  if ((send_patch_change) && (millis() > midi_timer)) { // We delay the sending of a PC message if these come to close together.
    ...
    midi_timer = millis() + ZMS_MIDI_TIMER_LENGTH;
    send_patch_change = false;
  }
}

FLASHMEM void MD_ZMS_class::set_type(uint8_t device_type) { // Is run just before we connect.
  model_number = device_type;
  switch (device_type) { // First determine which Fractal Audio device has connected...
    case ZMS50G_MODEL_NUMBER: // Zoom MS50G
      strcpy(device_name, "MS50G");
      strcpy(full_device_name, "Zoom MS50G");
      dev_type = 0;
      number_of_fx = 6;
      sysex_size = 146;
      break;
    case ZMS60B_MODEL_NUMBER: //Zoom MS60B
      strcpy(device_name, "MS60B");
      strcpy(full_device_name, "Zoom MS60B");
      dev_type = 1;
      number_of_fx = 4;
      sysex_size = 105;
      break;
    case ZMS70CDR_MODEL_NUMBER: //Zoom MS70-cdr
...
FLASHMEM uint8_t MD_ZMS_class::get_number_of_dev_types() {
  return 3;
}

FLASHMEM void MD_ZMS_class::get_dev_type_name(uint8_t number, String &name) {
  switch (number) {
    case 0: name = "Zoom MS50G"; break;
    case 1: name = "Zoom MS60B"; break;
    case 2: name = "Zoom MS70-cdr"; break;
    default: name = "?"; break;
  }
}

FLASHMEM void MD_ZMS_class::do_after_dev_type_update() {
  switch (dev_type) {
    case 0:
      model_number = ZMS50G_MODEL_NUMBER;
      set_type(model_number);
      break;
    case 1:
      model_number = ZMS60B_MODEL_NUMBER;
      set_type(model_number);
      break;
    case 2:
      model_number = ZMS70CDR_MODEL_NUMBER;
      set_type(model_number);
      break;
  }
}
...
FLASHMEM void MD_ZMS_class::check_SYSEX_in(const unsigned char* sxdata, short unsigned int sxlength, uint8_t port) { // Check incoming sysex messages from  Called from MIDI:OnSysEx/OnSerialSysEx

  // Check if it is a message from a ZOOM MS SERIES
  if ((port == MIDI_in_port) && (sxdata[1] == 0x52) && (sxdata[2] == MIDI_device_id) && (sxdata[3] == model_number)) {

    // Check if it is patch data for a specific patch
    if ((sxdata[4] == 0x08)  && (sxdata[7] == last_requested_sysex_patch_number)) {
      if  (sxlength == sysex_size + 10) { // Check if full sysex message is read...
        if (model_number != ZMS60B_MODEL_NUMBER) {
          ...
          // Read the current patch name - needs to be read in two parts, because the third byte in the patchname string is a 0.
          current_patch_name[0] = static_cast<char>(CP_MEM[96]);
          current_patch_name[1] = static_cast<char>(CP_MEM[97]);
          for (uint8_t count = 2; count < 9; count++) { // Read the last nine characters of the name
            current_patch_name[count] = static_cast<char>(CP_MEM[count + 97]); //Add byte 3 - 9 to ascii character to the SP.Label String
          }
          current_patch_name[9] = static_cast<char>(CP_MEM[107]);
        }
    ...
    if (sxdata[4] == 0x28) {
      if (sxlength == sysex_size) {
        ...
        if (model_number != ZMS60B_MODEL_NUMBER) {
          // Read the current patch name - needs to be read in two parts, because the eighth byte in the patchname string is a 0.
          ...
          // Read the current patch name - needs to be read in two parts, because the third byte in the patchname string is a 0.
          current_patch_name[0] = static_cast<char>(CP_MEM[91]);
          current_patch_name[1] = static_cast<char>(CP_MEM[92]);
          for (uint8_t count = 2; count < 9; count++) { // Read the last nine characters of the name
            current_patch_name[count] = static_cast<char>(CP_MEM[count + 92]); //Add byte 3 - 9 to ascii character to the SP.Label String
          }
          current_patch_name[9] = static_cast<char>(CP_MEM[102]);
        }
        ...
        update_page = REFRESH_FX_ONLY; // Need to update everything, otherwise displays go blank on detection of the MS SERIES.
      }
      else { // Data is not complete
        write_sysex(ZMS_REQUEST_CURRENT_PATCH); // Request current patch data again
      }
    }

    if ((sxdata[4] == 0x31) && (sxdata[6] == 0x00) && (sxlength == 10)) { // Check for effect switched off on the ZOOM MS SERIES.
...
FLASHMEM uint16_t MD_ZMS_class::FXtypeMangler(uint8_t byte1, uint8_t byte2, uint8_t byte4) {
...
// Detection of Zoom MS SERIES

FLASHMEM void MD_ZMS_class::identity_check(const unsigned char* sxdata, short unsigned int sxlength, uint8_t in_port, uint8_t out_port) {
  // Check if it is a Zoom MS SERIES
  if ((sxdata[5] == 0x52) &&  (sxdata[7] == 0x00) && (enabled == DEVICE_DETECT)) {
    if ((sxdata[6] == ZMS50G_MODEL_NUMBER) || (sxdata[6] == ZMS60B_MODEL_NUMBER) || (sxdata[6] == ZMS70CDR_MODEL_NUMBER)) {
      no_response_counter = 0;
      if (connected == false) {
        set_type(sxdata[6]);
        connect(sxdata[2], in_port, out_port); //Byte 2 contains the correct device ID
      }
    }
  }
}

FLASHMEM void MD_ZMS_class::do_after_connect() {
  is_on = true;
  write_sysex(ZMS_EDITOR_MODE_ON); // Put the Zoom MS SERIES in EDITOR mode
  write_sysex(ZMS_REQUEST_CURRENT_PATCH_NUMBER); // Receiving the current patch number will trigger a request for the current patch as well.
  CP_MEM_current = false;
  write_sysex(ZMS_REQUEST_CURRENT_PATCH); //This will update the FX buttons
  update_page = REFRESH_PAGE;
}

// ********************************* Section 3: ZOOM MS SERIES common MIDI out functions ********************************************

FLASHMEM void MD_ZMS_class::write_sysex(uint8_t message) {
  uint8_t sysexmessage[6] = {0xF0, 0x52, MIDI_device_id, model_number, message, 0xF7};
  //check_sysex_delay();
  MIDI_send_sysex(sysexmessage, 6, MIDI_out_port);
}

FLASHMEM void MD_ZMS_class::request_patch(uint8_t number) { //Will request the complete patch information from the Zoom MS SERIES (will receive 120 bytes as an answer)
  uint8_t sysexmessage[9] = {0xF0, 0x52, MIDI_device_id, model_number, 0x09, 0x00, 0x00, number, 0xF7};
  //check_sysex_delay();
  MIDI_send_sysex(sysexmessage, 9, MIDI_out_port);
}

FLASHMEM void MD_ZMS_class::set_FX_state(uint8_t number, uint8_t state) { //Will set an effect on or off
  uint8_t sysexmessage[10] = {0xF0, 0x52, MIDI_device_id, model_number, 0x31, number, 0x00, state, 0x00, 0xF7}; // F0 52 00 61 31 00 (FX) 00 01 (on) 00 F7
  //check_sysex_delay();
  MIDI_send_sysex(sysexmessage, 10, MIDI_out_port);
}

FLASHMEM void MD_ZMS_class::send_current_patch() { //  Send the previously saved data back to the Zoom unit
  if (CP_MEM_current) MIDI_send_sysex(CP_MEM, sysex_size, MIDI_out_port);
}

FLASHMEM void MD_ZMS_class::set_bpm() { //Will change the bpm to the specified value

  if ((connected) && (CP_MEM_current) && (model_number != ZMS60B_MODEL_NUMBER)) {
    write_tempo_to_cpmem();
    send_current_patch(); // Write the updated patch memory to the MS SERIES
  }
}

FLASHMEM void MD_ZMS_class::write_tempo_to_cpmem() {
...
FLASHMEM void MD_ZMS_class::start_tuner() {
  if (connected) {
    MIDI_send_CC(0x4A, 0x40, MIDI_channel, MIDI_out_port);
  }
}

FLASHMEM void MD_ZMS_class::stop_tuner() {
  if (connected) {
    MIDI_send_CC(0x4A, 0x00, MIDI_channel, MIDI_out_port);
  }
}

// ********************************* Section 4: MS SERIES program change ********************************************

FLASHMEM void MD_ZMS_class::select_patch(uint16_t new_patch) {
...
FLASHMEM void MD_ZMS_class::do_after_patch_selection() {
  is_on = connected;
  ...
  CP_MEM_current = false;
  write_sysex(ZMS_REQUEST_CURRENT_PATCH); // Request current patch, so the FX buttons will be updated
  MIDI_disable_device_check();
  MD_base_class::do_after_patch_selection();
}

FLASHMEM bool MD_ZMS_class::request_patch_name(uint8_t sw, uint16_t number) {
...
FLASHMEM void MD_ZMS_class::number_format(uint16_t number, String &Output) {
  Output += String((number + 1) / 10) + String((number + 1) % 10);
}

// ********************************* Section 5: MS SERIES parameter control ********************************************

// Define array for MS SERIES effeect names and colours
struct ZMS_FX_type_struct { // Combines all the data we need for controlling a parameter in a device
...
const PROGMEM ZMS_FX_type_struct ZMS_FX_types[] = { // Table with the name and colour for every effect of the Zoom MS SERIES
  {0, 0,    "---", FX_TYPE_OFF}, // 01
  ...
  // Unique effects from MS-60B that do not crash the MS SERIES
...
const uint8_t ZMS_NUMBER_OF_FX = sizeof(ZMS_FX_types) / sizeof(ZMS_FX_types[0]);

FLASHMEM uint8_t MD_ZMS_class::FXsearch(uint8_t type, uint8_t number) {
  uint8_t my_type = 0;
  for (uint8_t i = 0; i < ZMS_NUMBER_OF_FX; i++) {
    if ((ZMS_FX_types[i].Type == type) && (ZMS_FX_types[i].Number == number)) {
      my_type = i;
...
FLASHMEM void MD_ZMS_class::parameter_press(uint8_t Sw, Cmd_struct *cmd, uint16_t number) {
  ...
  if (CP_MEM_current) { // Here we change the on/off bit in the patch data memory and write it back to the unit
    switch (SP[Sw].PP_number) {
      ...
      case 4: if (number_of_fx == 6) bitWrite(CP_MEM[88], 0, FX_on); break;
      case 5: if (number_of_fx == 6) bitWrite(CP_MEM[108], 0, FX_on); break;
    }
    write_current_effect_focus_to_cpmem(SP[Sw].PP_number);
    write_tempo_to_cpmem();
    send_current_patch(); // Write the updated patch memory to the MS SERIES
  }
  else if (SP[Sw].PP_number < 3) { // This command will only work for the first three FX on the MS SERIES
...
FLASHMEM void MD_ZMS_class::parameter_release(uint8_t Sw, Cmd_struct *cmd, uint16_t number) {
  if (SP[Sw].Latch == MOMENTARY) {
    if (CP_MEM_current) { // Here we change the on/off bit in the patch data memory and write it back to the unit
      switch (SP[Sw].PP_number) {
        ...
        case 4: if (number_of_fx == 6) bitWrite(CP_MEM[88], 0, cmd->Value2); break;
        case 5: if (number_of_fx == 6) bitWrite(CP_MEM[108], 0, cmd->Value2); break;
      }
      write_current_effect_focus_to_cpmem(SP[Sw].PP_number);
      write_tempo_to_cpmem();
      send_current_patch(); // Write the updated patch memory to the MS SERIES
...
FLASHMEM void MD_ZMS_class::write_current_effect_focus_to_cpmem(uint8_t fx_no) {
  ...
  if ((fx_no < 4) && (number_of_fx == 4)) { // MS60B
    bitWrite(CP_MEM[85], 3, (bitState[fx_no] & B010) >> 1);
    bitWrite(CP_MEM[88], 6, bitState[fx_no] & B001);
  }
}

// Parameters are the 6 FX buttons
FLASHMEM void MD_ZMS_class::read_parameter_title(uint16_t number, String &Output) {
  Output += "FX " + String(number + 1);
}

FLASHMEM void MD_ZMS_class::read_parameter_title_short(uint16_t number, String &Output) {
  uint8_t FX_type = FX[number] >> 1; //The FX type is stored in bit 1-7.
  Output += String(number + 1) + ':' + ZMS_FX_types[FX_type].Name;
}

FLASHMEM bool MD_ZMS_class::request_parameter(uint8_t sw, uint16_t number) {
  //Effect type and state are stored in the ZMS_FX array
  //Effect can have three states: 0 = no effect, 1 = on, 2 = off
  if (number < number_of_fx) {
    if (FX[number] & 0x01) SP[sw].State = 1; //Effect on
    else SP[sw].State = 2; // Effect off
  }

  uint8_t FX_type = FX[number] >> 1; //The FX type is stored in bit 1-7.
  LCD_set_SP_label(sw, ZMS_FX_types[FX_type].Name); //Find the patch name in the ZMS_FX_types array
  SP[sw].Colour = ZMS_FX_types[FX_type].Colour; //Find the LED colour in the ZMS_FX_types array
  return true; // Move to next switch is true
}

// Menu options for FX states
FLASHMEM void MD_ZMS_class::read_parameter_name(uint16_t number, String &Output) { // Called from menu
  if (number < number_of_parameters())  Output = "FX" + String(number + 1) + " SW";
  else Output = "?";
}

FLASHMEM uint16_t MD_ZMS_class::number_of_parameters() {
  return number_of_fx;
}

FLASHMEM uint8_t MD_ZMS_class::number_of_values(uint16_t parameter) {
  if (parameter < number_of_parameters()) return 2; // So far all parameters have two states: on and off
  else return 0;
}

FLASHMEM void MD_ZMS_class::read_parameter_value_name(uint16_t number, uint16_t value, String &Output) {
