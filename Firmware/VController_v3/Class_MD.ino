...
// Section 6: MD_ZMS_class declaration (derived)
...
// Section 16: MD_USER_class declaration (derived)
...
{ // Base class for midi devices
  ...
  public:
    ...
    virtual bool can_request_sysex_data();
    ...
    void build_patch_number(uint16_t number, String &Output, String firstPatch, String lastPatch);
    uint8_t convert_mask_number(char c);
    ...
    uint8_t current_device_setlist = 0; // Needed for Helix and AxeFX
...
class MD_ZG3_class : public MD_base_class
{
  public:
    ...
    // Variables:
    const uint8_t NUMBER_OF_FX_SLOTS = 6; // The Zoom MS SERIES has 6 FX slots
...
class MD_ZMS_class : public MD_base_class
{
  public:
    MD_ZMS_class (uint8_t _dev_no) : MD_base_class (_dev_no) {} // Constructor
    ...
    virtual void parameter_press(uint8_t Sw, Cmd_struct *cmd, uint16_t number);
    virtual void parameter_release(uint8_t Sw, Cmd_struct *cmd, uint16_t number);
    void write_current_effect_focus_to_cpmem(uint8_t fx_no);
    virtual void read_parameter_name(uint16_t number, String &Output);
    virtual uint16_t number_of_parameters();
    virtual uint8_t number_of_values(uint16_t parameter);
    virtual void read_parameter_value_name(uint16_t number, uint16_t value, String &Output);
    ...
    // Variables
    uint8_t model_number = 0x61; // MS70cdr by default
    uint8_t number_of_fx = 6;
    uint8_t sysex_size = 146;
    const uint8_t NUMBER_OF_FX_SLOTS = 6; // The Zoom MS SERIES has 6 FX slots
    uint16_t FX[6]; //Memory location for the 6 FX
#define ZMS_CURRENT_PATCH_DATA_SIZE 146
    uint8_t CP_MEM[ZMS_CURRENT_PATCH_DATA_SIZE]; // Memory to store the current patch data
    bool CP_MEM_current;
    bool send_patch_change = false;
};
...
class MD_USER_class : public MD_base_class
{
  public:
    MD_USER_class (uint8_t _dev_no) : MD_base_class (_dev_no) {} // Constructor
    ...
    virtual uint16_t number_of_parameters();
    virtual uint8_t number_of_values(uint16_t parameter);
    uint8_t get_toggle_type(uint16_t parameter);
    void set_par_data(uint8_t sw, uint16_t par_id);
    void update_parameter_state_through_cc(uint8_t control, uint8_t value);
    uint8_t get_par_state();
    void save_par_state();
    void load_par_state();
    uint8_t load_fx_colour(uint16_t parameter);
    bool check_cc_not_zero(uint8_t parameter);
    ...
    void edit();
    void edit_done();
    //void read_string_from_device_data(uint8_t index, uint8_t length, String &str);
    //void store_string_to_device_data(uint8_t index, uint8_t length, String str);
...
#define USER_PAR_LOOPER_PLAY 15
#define USER_PAR_LOOPER_REC 16
