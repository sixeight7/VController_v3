...
void MD_MG300_class::init() { // Default values for variables
  MD_base_class::init();
  ...
  MIDI_port = MG300_MIDI_PORT; // Default value
  my_device_page1 = MG300_DEFAULT_PAGE1; // Default value
  my_device_page2 = MG300_DEFAULT_PAGE2; // Default value
  my_device_page3 = MG300_DEFAULT_PAGE3; // Default value
  my_device_page4 = MG300_DEFAULT_PAGE4; // Default value

}

void MD_MG300_class::update() {
...
// ********************************* Section 2: MG300 common MIDI in functions ********************************************
void MD_MG300_class::check_SYSEX_in(const unsigned char* sxdata, short unsigned int sxlength, uint8_t port) { // Check incoming sysex messages from  Called from MIDI:OnSysEx/OnSerialSysEx
...
void MD_MG300_class::read_patch_name(const unsigned char* sxdata, short unsigned int sxlength) {
  uint8_t len = sxdata[0x4D]; // Length of patch name is variable.
  current_checksum = 0;
  if (len > 16) len = 16;
  for (uint8_t count = 0; count < len; count++) {
...
void MD_MG300_class::check_CC_in(uint8_t control, uint8_t value, uint8_t channel, uint8_t port) {
  if (!connected) return;
...
void MD_MG300_class::send_alternative_identity_request(uint8_t check_device_no) {
  if (enabled == DEVICE_DETECT) { // F0 00 11 22 66 00 00 00  00 F7
    ...
    check_sysex_delay();
    if (check_device_no == 0 ) MIDI_send_sysex(sysexmessage, 10, USBMIDI_PORT);
    if (check_device_no == 1 ) MIDI_send_sysex(sysexmessage, 10, MIDI1_PORT);
    if (check_device_no == 2 ) MIDI_send_sysex(sysexmessage, 10, MIDI2_PORT);
    if (check_device_no == 3 ) MIDI_send_sysex(sysexmessage, 10, MIDI3_PORT);
    if (check_device_no == 4 ) MIDI_send_sysex(sysexmessage, 10, USBHMIDI_PORT);
  }
}

void MD_MG300_class::do_after_connect() {
...
void MD_MG300_class::request_patch(uint8_t number) { //Will request the complete patch information from the MG300 (will receive 120 bytes as an answer)
...
void MD_MG300_class::request_current_patch() { //Will request the complete patch information from the MG300 (will receive 120 bytes as an answer)
...
void MD_MG300_class::request_device_state() {
...
void MD_MG300_class::select_patch(uint16_t new_patch) {
  //if (new_patch == patch_number) unmute();
  prev_patch_number = patch_number;
  patch_number = new_patch;

  MIDI_send_CC(60, new_patch, MIDI_channel, MIDI_port);
  DEBUGMSG("out(MG300) CC #60 " + String(new_patch)); //Debug
  do_after_patch_selection();
  update_page = REFRESH_PAGE;
}

void MD_MG300_class::do_after_patch_selection() {
  is_on = connected;
...
bool MD_MG300_class::request_patch_name(uint8_t sw, uint16_t number) {
...
void MD_MG300_class::number_format(uint16_t number, String & Output) {
...
void MD_MG300_class::direct_select_format(uint16_t number, String & Output) {
...
bool MD_MG300_class::flash_LEDs_for_patch_bank_switch(uint8_t sw) { // Will flash the LEDs in banks of three when coming from direct select mode.
...
bool MD_MG300_class::valid_direct_select_switch(uint8_t number) {
  bool result = false;
...
void MD_MG300_class::direct_select_start() {
...
void MD_MG300_class::direct_select_press(uint8_t number) {
  ...
  SCO_select_page(MG300_DEFAULT_PAGE1); // Which should give PAGE_MG300_PATCH_BANK
...
struct MG300_CC_type_struct { // Combines all the data we need for controlling a parameter in a device
  char Name[6]; // The name for the label
...
const PROGMEM char MG300_sublists[][11] = {
  ...
  "SUPER RVB", "BRIT BLUES", "MATCH D30", "BRIT 2000", "UBER HIGN",
...
void MD_MG300_class::parameter_press(uint8_t Sw, Cmd_struct *cmd, uint16_t number) {
  ...
  if (value == 1) {
    ...
    MIDI_send_CC(MG300_CC_types[number].CC, on_value, MIDI_channel, MIDI_port);
  }
  else {
    MIDI_send_CC(MG300_CC_types[number].CC, 0, MIDI_channel, MIDI_port);
  }
  fx_state[number] = value;
  if (number == 0) MIDI_send_CC(0, fx_type[0], MIDI_channel, MIDI_port); // Reset expression pedal type to original value after switching wah
  if (number < 9) MIDI_send_CC(66, number, MIDI_channel, MIDI_port); // Select this effect in edit mode on the MG300
...
void MD_MG300_class::parameter_release(uint8_t Sw, Cmd_struct *cmd, uint16_t number) {
...
void MD_MG300_class::read_parameter_title(uint16_t number, String &Output) {
  Output += MG300_CC_types[number].Name;
}

bool MD_MG300_class::request_parameter(uint8_t sw, uint16_t number) {
  String msg = "";
...
void MD_MG300_class::read_parameter_name(uint16_t number, String &Output) { // Called from menu
  if (number < number_of_parameters())  Output = MG300_CC_types[number].Name;
  else Output = "?";
}

uint16_t MD_MG300_class::number_of_parameters() {
  return MG300_NUMBER_OF_FX;
}

uint8_t MD_MG300_class::number_of_values(uint16_t parameter) {
...
void MD_MG300_class::read_parameter_value_name(uint16_t number, uint16_t value, String &Output) {
  if (number < number_of_parameters()) Output += String(value);
  else Output += " ? "; // Unknown parameter
}

void MD_MG300_class::update_parameter_state_through_cc(uint8_t control, uint8_t value) {
...
void MD_MG300_class::read_FX_state_from_memory() {
...
/*void MD_MG300_class::move_expression_pedal(uint8_t sw, uint8_t value, uint8_t exp_pedal) {
  //uint8_t number;
  //number = 0;
  LCD_show_bar(0, value); // Show it on the main display
  MIDI_send_CC(61, value, MIDI_channel, MIDI_port);
...
bool MD_MG300_class::request_exp_pedal(uint8_t sw, uint8_t exp_pedal) {
  SP[sw].PP_number = 0;
