...
// Section 1: AXEFX Initialization
// Section 2: AXEFX common MIDI in functions
// Section 3: AXEFX common MIDI out functions
// Section 4: AXEFX program change
// Section 5: AXEFX parameter control
// Section 6: AXEFX assign control
...
// CC messages
#define AXE_BYPASS_CC 13
#define AXE_TUNER_CC 15
#define AXE_SCENE_SELECT_CC 34 // Default value for scene select - but not on AF3

// Block IDs
#define AXE_LOOPER_ID 169
...
#define AXE_FIRMWARE_VERSION 0x08
#define AXE_FIRMWARE_VERSION_AF2 0x64
#define AXE_GET_PRESET_NUMBER 0x14
#define AXE_GET_PRESET_NAME 0x0F
#define AXE_GET_PRESET_NAME_BY_NUMBER 0x15
#define AXE_EDIT_MODE_ON 0x08
#define AXE_EDIT_MODE_OFF 0x42
#define AXE_GET_LOOPER_STATUS 0x23
#define AXE_GET_SCENE_STATUS 0x29
#define AXE_REALTIME_TEMPO 0x10
#define AXE_REALTIME_TUNER 0x0D
#define AXE_SET_PARAMETER 0x02
#define AXE_GET_FX_BLOCK_BYPASS_STATE 0x0E
#define AXE_GET_SET_BLOCK_PARAMETER_VALUE 0x02

// Initialize device variables
// Called at startup of VController
void AXEFX_class::init() { // Default values for variables
  ...
  MIDI_port = AXEFX_MIDI_PORT; // Default value
...
void AXEFX_class::check_SYSEX_in(const unsigned char* sxdata, short unsigned int sxlength, uint8_t port) { // Check incoming sysex messages from  Called from MIDI:OnSysEx/OnSerialSysEx
  ...
  if ((sxdata[1] == 0x00) && (sxdata[2] == 0x01) && (sxdata[3] == 0x74)) {
    ...
    if (((sxdata[5] == AXE_FIRMWARE_VERSION) || (sxdata[5] == AXE_FIRMWARE_VERSION_AF2)) && (!connected)) {
      ...
      switch (sxdata[5]) {
        ...
        case AXE_REALTIME_TEMPO: // We use the realtime tempo messages to check if the AxeFX is still connected
          no_response_counter = 0;
          break;

        case AXE_GET_PRESET_NUMBER:
        ...
        case AXE_GET_PRESET_NAME_BY_NUMBER: // Patch name of specific preset
        ...
        case AXE_GET_FX_BLOCK_BYPASS_STATE: // Check if it is a SYSEX_AXEFX_GET_PRESET_EFFECT_BLOCKS_AND_CC_AND_BYPASS_STATE message
...
void AXEFX_class::check_PC_in(uint8_t program, uint8_t channel, uint8_t port) {  // Check incoming PC messages from  Called from MIDI:OnProgramChange
  ...
  if ((port == MIDI_port) && (channel == MIDI_channel)) { // AXEFX sends a program change
    ...
    if (patch_number != new_patch) {
      patch_number = new_patch;
      write_sysex(AXE_GET_PRESET_NAME); // So the main display always show the correct patch
      //page_check();
      do_after_patch_selection();
      update_page = REFRESH_PAGE;
    }
...
void AXEFX_class::send_alternative_identity_request(uint8_t check_device_no) {
  if ((enabled == DEVICE_DETECT) && (!connected)) {
    uint8_t sysexmessage[8] = {0xF0, 0x00, 0x01, 0x74, 0x7F, 0x00, 0x7A, 0xF7}; // Request any fractal device
    check_sysex_delay();
...
void AXEFX_class::do_after_connect() {
  current_exp_pedal = 1;
  write_sysex(AXE_GET_PRESET_NUMBER);
  write_sysex(AXE_GET_PRESET_NAME); // So the main display always show the correct patch
  //write_sysex(AXEFX_EDITOR_MODE_ON); // Put the AXEFX in EDITOR mode
  do_after_patch_selection();
  update_page = REFRESH_PAGE;
}

void AXEFX_class::check_still_connected() {} // Never disconnect

// ********************************* Section 3: AXEFX common MIDI out functions ********************************************

void AXEFX_class::write_sysex(uint8_t byte1) {
...
void AXEFX_class::write_sysex(uint8_t byte1, uint8_t byte2) {
...
void AXEFX_class::write_sysex(uint8_t byte1, uint8_t byte2, uint8_t byte3) {
...
void AXEFX_class::write_sysex(uint8_t byte1, uint8_t byte2, uint8_t byte3, uint8_t byte4, uint8_t byte5, uint8_t byte6, uint8_t byte7, uint8_t byte8, uint8_t byte9) {
...
uint8_t AXEFX_class::calc_FS_checksum(const unsigned char* sxdata, short unsigned int sxlength) {
...
#define CONTROLLERS_BLOCK_ID  141
  ...
  if (connected) {
    write_sysex(AXE_GET_SET_BLOCK_PARAMETER_VALUE, CONTROLLERS_BLOCK_ID & 0x7F, CONTROLLERS_BLOCK_ID >> 7, 32, 0, Setting.Bpm & 0x7F, Setting.Bpm >> 7, 0, 1);
  }
}

void AXEFX_class::start_tuner() {
  if (connected) {
    MIDI_send_CC(AXE_TUNER_CC, 127, MIDI_channel, MIDI_port);
  }
}

void AXEFX_class::stop_tuner() {
  if (connected) {
    MIDI_send_CC(AXE_TUNER_CC, 0, MIDI_channel, MIDI_port);
  }
}

// ********************************* Section 4: AXEFX program change ********************************************

void AXEFX_class::select_patch(uint16_t new_patch) {
...
void AXEFX_class::do_after_patch_selection() {
  request_onoff = false;
  is_on = connected;
  if (Setting.Send_global_tempo_after_patch_change == true) {
    set_bpm();
  }
  write_sysex(AXE_GET_FX_BLOCK_BYPASS_STATE); // Request the states of the effects
  write_sysex(AXE_GET_SCENE_STATUS, 0x7F); // Request the current scene
  ...
  update_main_lcd = true;
  //update_page = REFRESH_PAGE;
  //EEPROM.write(EEPROM_AXEFX_PATCH_NUMBER, patch_number);
...
bool AXEFX_class::request_patch_name(uint8_t sw, uint16_t number) {
  uint8_t msb = number >> 7;
  uint8_t lsb = number & 0x7F;
  write_sysex(AXE_GET_PRESET_NAME_BY_NUMBER, msb, lsb); // F0 00 01 74 03 15 7F 7F 13 F7
  last_requested_sysex_type = REQUEST_PATCH_NAME;
  last_requested_sysex_switch = sw;
  return false;
}

void AXEFX_class::request_current_patch_name() {
  write_sysex(AXE_GET_PRESET_NAME);
}

void AXEFX_class::number_format(uint16_t number, String & Output) {
...
void AXEFX_class::direct_select_format(uint16_t number, String & Output) {
...
void AXEFX_class::unmute() {
  is_on = connected;
  MIDI_send_CC(AXE_BYPASS_CC, 0, MIDI_channel, MIDI_port);
}

void AXEFX_class::mute() {
  if ((Setting.US20_emulation_active) && (!is_always_on) && (is_on)) {
    is_on = false;
    MIDI_send_CC(AXE_BYPASS_CC, 127, MIDI_channel, MIDI_port);
  }
}
...
// Whenever the patch is changed on the AxeFX, we send the AXE_GET_FX_BLOCK_BYPASS_STATE command
...
// Procedures for the AXEFX_PARAMETER:
// 1. Load in SP array L load_page()
// 2. Request parameter state - in PAGE_request_current_switch()
// 3. Read parameter state - AXEFX_read_parameter() below
// 4. Press switch - AXEFX_parameter_press() below - also calls check_update_label()
// 5. Release switch - AXEFX_parameter_release() below - also calls check_update_label()
...
const PROGMEM AXEFX_parameter_struct AXEFX_parameters[] = {
...
const uint16_t AXEFX_NUMBER_OF_PARAMETERS = sizeof(AXEFX_parameters) / sizeof(AXEFX_parameters[0]);

#define AXE_FIRST_EXTERNAL_PEDAL 85
#define AXE_FIRST_EXTERNAL_PEDAL_CC 16

void AXEFX_class::read_parameter_name(uint16_t number, String & Output) { // Called from menu
  if (number < number_of_parameters())  Output = AXEFX_parameters[number].Name;
  else Output = "?";
}

void AXEFX_class::read_parameter_value_name(uint16_t number, uint16_t value, String & Output) { // Called from menu
  if (number < AXE_FIRST_EXTERNAL_PEDAL) { // Show ON/OFF for IA and XY switches
    if (value == 1) Output += "ON";
    else Output += "OFF";
    return;
  }
  if (number < AXEFX_NUMBER_OF_PARAMETERS) { // External pedal
...
void AXEFX_class::clear_FX_states() {
  memset(effect_state, 0, AXEFX_NUMBER_OF_FX);
}

void AXEFX_class::set_FX_state(uint8_t cc, bool state) {
  if (cc < AXE_FX_FIRST_CC) return; // Exit if the CC is smaller than AXE_FX_FIRST_CC (37)
  uint8_t index = cc - AXE_FX_FIRST_CC;
  if (index < AXEFX_NUMBER_OF_FX) effect_state[index] = (state == 0) ? 2 : 1; // Set state to 2 if state is zero and to 1 if state is not zero
}

// Toggle AXEFX stompbox parameter
void AXEFX_class::parameter_press(uint8_t Sw, Cmd_struct * cmd, uint16_t number) {

  if (number >= AXEFX_NUMBER_OF_PARAMETERS) return; // Quit if number is out of range

  uint8_t value = SCO_return_parameter_value(Sw, cmd); // Value is toggled already

  // Send cc MIDI command to AXEFX
  uint8_t cc;
  if (number < AXE_FIRST_EXTERNAL_PEDAL) { // IA or XY switch
    cc = number + AXE_FX_FIRST_CC;
    if (value == 1) MIDI_send_CC(cc, 127, MIDI_channel, MIDI_port);
    else  MIDI_send_CC(cc, 0, MIDI_channel, MIDI_port);
  }
  else { // External pedal
    cc = number - AXE_FIRST_EXTERNAL_PEDAL + AXE_FIRST_EXTERNAL_PEDAL_CC;
...
void AXEFX_class::parameter_release(uint8_t Sw, Cmd_struct * cmd, uint16_t number) {
  // Work out state of pedal
  if (SP[Sw].Latch == MOMENTARY) {
    effect_state[number] = 2; // Switch state off
    SP[Sw].State = 2;
    uint8_t cc = number + AXE_FX_FIRST_CC;
...
bool AXEFX_class::request_parameter(uint8_t sw, uint16_t number) {
  ...
  if (number < AXEFX_NUMBER_OF_PARAMETERS) {
    if (number < AXE_FIRST_EXTERNAL_PEDAL) { // IA or XY switch
      ...
      msg = device_name;
      msg += ' ';
      msg += AXEFX_parameters[number].Name;
      msg += ":";
      LCD_add_3digit_number(effect_state[number], msg);
      LCD_set_SP_label(sw, msg);
    }

    SP[sw].Colour = AXEFX_parameters[number].Colour; // Set the FX colour
  }

  return true; // Move to next switch is true
}

void AXEFX_class::check_update_label(uint8_t Sw, uint8_t value) { // Updates the label for extended sublists
  uint16_t index = SP[Sw].PP_number; // Read the parameter number (index to AXEFX-parameter array)
  if ((index >= AXE_FIRST_EXTERNAL_PEDAL) && (index < AXEFX_NUMBER_OF_PARAMETERS)) {
    String msg;
    msg = device_name;
    msg += ' ';
    msg += AXEFX_parameters[index].Name;
    msg += ":";
...
uint8_t AXEFX_class::number_of_values(uint16_t parameter) {
  if (parameter < AXE_FIRST_EXTERNAL_PEDAL) return 2; // IA or XY switch
  if (parameter < AXEFX_NUMBER_OF_PARAMETERS) return 128; // External pedal
  return 0; // Out of range
}

uint16_t AXEFX_class::number_of_parbank_parameters() {
  if (enabled == DEVICE_DETECT) return number_of_active_blocks * 2; // Every active block has a bypass and an XY state
  else return AXEFX_NUMBER_OF_PARAMETERS; // When not connected bi-directionally, just show all the parameters
}

uint16_t AXEFX_class::get_parbank_parameter_id(uint16_t par_number) {
  //Find the correct parameter number for this parameter number
  if (enabled == DEVICE_DETECT) {
    uint8_t active_fx_number = 0;
    for (uint8_t i = 0; i < AXEFX_NUMBER_OF_FX; i++) { // We go through the effect_state array and look for effects that are enabled
...
void AXEFX_class::move_expression_pedal(uint8_t sw, uint8_t value, uint8_t exp_pedal) {
  if (exp_pedal == 0) exp_pedal = current_exp_pedal;
  if (exp_pedal == 0) return;
  LCD_show_bar(0, value); // Show it on the main display
  MIDI_send_CC(AXE_FIRST_EXTERNAL_PEDAL_CC + exp_pedal - 1, value, MIDI_channel, MIDI_port);
  check_update_label(sw, value);
  LCD_show_status_message(SP[sw].Label);
  update_page = REFRESH_PAGE; // To update the other switch states, we re-load the current page
}

bool AXEFX_class::request_exp_pedal(uint8_t sw, uint8_t exp_pedal) {
  if (exp_pedal == 0) exp_pedal = current_exp_pedal;
  if (exp_pedal > 0) {
    uint8_t number = AXE_FIRST_EXTERNAL_PEDAL + exp_pedal - 1;
...
void AXEFX_class::set_snapscene_name(uint8_t number, String &Output) {
  Output += "SCENE " + String(number);
}

void AXEFX_class::set_snapscene(uint8_t number) {
  current_snapscene = number;
  MIDI_send_CC(AXE_SCENE_SELECT_CC, number - 1, MIDI_channel, MIDI_port);
}

bool AXEFX_class::looper_active() { // Is there a way to detect if the current patch contains a looper block?
  return looper_block_detected;
}

struct AXEFX_looper_cc_struct { // Combines all the data we need for controlling a parameter in a device
  uint8_t cc; // The cc number
  uint8_t value; // The value.
};

const PROGMEM AXEFX_looper_cc_struct AXEFX_Ultra_looper1_cc[] = { // Table with the cc messages for looper 1 of the AxeFX ultra
  {0, 0},    // HIDE
  {0, 0},    // SHOW
  {25, 0},   // STOP
  {25, 127}, // PLAY
...
const uint8_t AXEFXU_LOOPER1_NUMBER_OF_CCS = sizeof(AXEFX_Ultra_looper1_cc) / sizeof(AXEFX_Ultra_looper1_cc[0]);

const PROGMEM AXEFX_looper_cc_struct AXEFX_Ultra_looper2_cc[] = { // Table with the cc messages for looper 2 of the AxeFX ultra
  {0, 0},    // HIDE
  {0, 0},    // SHOW
  {30, 0},   // STOP
  {30, 127}, // PLAY
...
const uint8_t AXEFXU_LOOPER2_NUMBER_OF_CCS = sizeof(AXEFX_Ultra_looper2_cc) / sizeof(AXEFX_Ultra_looper2_cc[0]);

const PROGMEM AXEFX_looper_cc_struct AXEFX2_looper_cc[] = { // Table with the cc messages for the AxeFX 2 and up, FX8 and AX8
  {33, 0},    // HIDE
  {33, 127},  // SHOW
  {29, 0},    // STOP
  {29, 127},  // PLAY
...
void AXEFX_class::send_looper_cmd(uint8_t cmd) {
  ...
  if ((model_number == AXE_MODEL_ULTRA) && (looper_pre)) {
    if (cmd < AXEFXU_LOOPER1_NUMBER_OF_CCS) {
      if (AXEFX_Ultra_looper1_cc[cmd].cc > 0) MIDI_send_CC(AXEFX_Ultra_looper1_cc[cmd].cc, AXEFX_Ultra_looper1_cc[cmd].value, MIDI_channel, MIDI_port);
    }
  }
  // Execute commands for looper 2 of the AxeFX Ultra if looper_pre is off
  if ((model_number == AXE_MODEL_ULTRA) && (!looper_pre)) {
    if (cmd < AXEFXU_LOOPER2_NUMBER_OF_CCS) {
      if (AXEFX_Ultra_looper2_cc[cmd].cc > 0) MIDI_send_CC(AXEFX_Ultra_looper2_cc[cmd].cc, AXEFX_Ultra_looper2_cc[cmd].value, MIDI_channel, MIDI_port);
    }
  }
}
