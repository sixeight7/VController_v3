...
const PROGMEM menu_struct menu[][15] = {
  { // Menu 0 - Select menu
    ...
    { "CALIBRATION MENU", OPEN_MENU, CALIBRATION_MENU }, // Switch 5
    { "", NONE }, // Switch 7
    { "", NONE }, // Switch 8
    { "", NONE }, // Switch 9
    { "", NONE }, // Switch 10
  ...
  { // Menu 1 - Global settings
    ...
    { "Main disply mode", SET, 20, 0, 2, &Setting.Main_display_mode },// Switch 3
  ...
  { // Menu 2 - Device settings
    ...
    { "Device page #1", DEVICE_SET, PAGE_SUBLIST, 0, LAST_FIXED_CMD_PAGE, (void*) 6 },// Switch 5
    { "Device page #2", DEVICE_SET, PAGE_SUBLIST, 0, LAST_FIXED_CMD_PAGE, (void*) 7 }, // Switch 6
    { "Device page #3", DEVICE_SET, PAGE_SUBLIST, 0, LAST_FIXED_CMD_PAGE, (void*) 8 }, // Switch 7
    { "Device page #4", DEVICE_SET, PAGE_SUBLIST, 0, LAST_FIXED_CMD_PAGE, (void*) 9 }, // Switch 8
  ...
  { // Menu 3 - LED settings menu
    ...
    { "LED Brightness", SET, 0, 0, 100, &Setting.LED_brightness }, // Switch 2
    { "Backlight Bright", SET, 0, 0, 255, &Setting.Backlight_brightness }, // Switch 3
    { "Virtual LEDs", SET, 1, 0, 1, &Setting.Virtual_LEDs },// Switch 4
    { "FX off is dimmed", SET, 1, 0, 1, &Setting.LED_FX_off_is_dimmed }, // Switch 5
    { "Global colour", SET, 4, 0, NUMBER_OF_SELECTABLE_COLOURS - 1, &Setting.LED_global_colour }, // Switch 6
    { "BPM colour", SET, 4, 0, NUMBER_OF_SELECTABLE_COLOURS - 1, &Setting.LED_bpm_colour }, // Switch 7
    { "MIDI PC colour", SET, 4, 0, NUMBER_OF_SELECTABLE_COLOURS - 1, &Setting.MIDI_PC_colour }, // Switch 8
    { "MIDI CC colour", SET, 4, 0, NUMBER_OF_SELECTABLE_COLOURS - 1, &Setting.MIDI_CC_colour }, // Switch 9
    { "MIDI note colour", SET, 4, 0, NUMBER_OF_SELECTABLE_COLOURS - 1, &Setting.MIDI_note_colour }, // Switch 10
    { "SAVE & EXIT", SAVE_AND_EXIT, 1 }, // Switch 11
    { "Cancel", SAVE_AND_EXIT, 0 }, // Switch 12
    { "", NONE }, // Switch 13 (LEFT)
    { "", NONE }, // Switch 14 (RIGHT)
  ...
  { // Menu 4 - LED effect colours
    ...
    { "DEFAULT COLOUR", SET, 4, 0, NUMBER_OF_SELECTABLE_COLOURS - 1, &Setting.FX_default_colour }, // Switch 10
    { "", NONE }, // Switch 11
  ...
  { // Menu 11 - Katana menu
    ...
    { "Rename",  EXECUTE, 0, 0, 0, (void*)KTN_rename }, // Switch 2
    { "Write",  EXECUTE, 0, 0, 0, (void*)KTN_save },// Switch 3
    { "Cancel", EXECUTE, 0, 0, 0, (void*)KTN_exit }, // Switch 4
...
void initialize_settings() {
  if (menu_are_you_sure("All settings are", "reset! Sure?")) {
...
void menu_open() { // Called when the menu is started the first time
  ...
  else current_menu = SELECT_MENU; // Go to top menu
  no_hold = true;
}
...
void menu_move_expr_pedal(uint8_t value) { // Called when the master expression pedal is moved
  ...
  switch (menu[current_menu][number].Type) {
    case CMD_BYTE:
      cmdbyte_from_exp_pedal(value);
      break;
    case SET:
      LCD_show_bar(Last_set_switch, value); // Show it on the switch display
      //LCD_show_status_message(menu[current_menu][number].Label);
      val = reinterpret_cast<uint8_t*>(menu[current_menu][number].Target);
      *val = map (value, 0, 127, menu[current_menu][number].Min, menu[current_menu][number].Max);
      ...
      menu_load(Last_set_switch); // Will update the label
      no_hold = false;
      break;
    case DEVICE_SET:
      LCD_show_bar(Last_set_switch, value); // Show it on the switch display
...
const PROGMEM char cmd_sublist[][17] = {
  ...
  // Sublist 90 - 97: CC toggle types
  "OFF", "ON_OFF", "PLAY_STOP", "REC_OVERDUB", "UNDO_REDO", "HALF_SPEED", "REVERSE", "PLAY_ONCE",
...
void load_cmd_byte(uint8_t Sw, uint8_t number) { // Display a command byte on a display.
  ...
  strcpy(SP[Sw].Title, cmdtype[cmd_type].Title); // Copy the title from the cmdbyte array
...
void read_cmd_sublist(uint8_t cmd_type, uint8_t value, String &msg) {
  ...
  switch (cmdtype[cmd_type].Sublist) { // Show sublist if neccesary
    ...
    case SUBLIST_PAR_STATE: // Copy the parameter state from the device
      dev = cmdbyte[0].Value;
      msg = "";
      if (dev < NUMBER_OF_DEVICES) Device[dev]->read_parameter_value_name(cmdbyte[2].Value, value, msg);
      else msg = String(value);
      break;
...
void build_command_structure(uint8_t cmd_byte_no, uint8_t cmd_type, bool in_edit_mode) {
  ...
  if ((cmd_byte_no >= 4) && (cmd_byte_no < 8)) {
    // *****************************************
    ...
    if (cmd_type == TYPE_PAR_STATE) {
      ...
      if (dev < NUMBER_OF_DEVICES) {
        // Check if we have reached the max value
        if (cmdbyte[cmd_byte_no].Value >= Device[dev]->number_of_values(cmdbyte[2].Value)) cmdbyte[cmd_byte_no].Value = 0;
      }
    }
  }
...
void cmdbyte_from_exp_pedal(uint8_t value) { // Set the value of a command byte from the expression pedal
  ...
  if (cmdtype[cmd_type].Max > 0) {
    ...
    //LCD_show_status_message(cmdtype[cmd_type].Title);
    build_command_structure(cmd_byte_no, cmd_type, true);
    menu_load(Last_set_switch); // So the correct text shows below the display
    LCD_show_bar(Last_set_switch, value); // Show it on the Sw display
  }
}
...
void menu_set_main_title() { // Called from main_LCD_control() when the main display is updated and it the page is the menu page
  ...
  switch (current_menu) {
    ...
    case COMMAND_EDIT_MENU:
      LCD_main_set_label("Pg:" + String(cmdbyte[8].Value) + " Sw:" + String(cmdbyte[9].Value) + " Cmd:" + String(cmdbyte[10].Value + 1));
      break;
    case KEYBOARD_MENU:
