...
// The Device_class is the base class for all the devices
...
void Device_class::forward_MIDI_message(const unsigned char* sxdata, short unsigned int sxlength) {}
...
void Device_class::forward_PC_message(uint8_t program, uint8_t channel) {}
...
void Device_class::check_manual_connection() {
  ...
  if (enabled == DEVICE_ON) { // We always have to set the port, when enabled == DEVICE_ON, so a change of manual port will update the device port
    if (MIDI_port_manual == 0) MIDI_port = USBMIDI_PORT;
    if (MIDI_port_manual == 1) MIDI_port = MIDI1_PORT;
    if (MIDI_port_manual == 2) MIDI_port = MIDI2_PORT;
    if (MIDI_port_manual == 3) MIDI_port = MIDI3_PORT;
    if (MIDI_port_manual == 4) MIDI_port = ALL_PORTS;
...
bool Device_class::can_request_sysex_data() {
  if (enabled == DEVICE_DETECT) return connected;
  else return false;
}

void Device_class::send_alternative_identity_request(uint8_t check_device_no) {}
...
void Device_class::do_after_disconnect() {}
...
void Device_class::bpm_tap() {}
void Device_class::start_tuner() {}
void Device_class::stop_tuner() {}

// ********************************* Section 4: Device program change ********************************************

void Device_class::select_patch(uint16_t new_patch) {
...
bool Device_class::request_patch_name(uint8_t sw, uint16_t number) {
...
void Device_class::patch_select_pressed(uint16_t new_patch) {
...
void Device_class::update_bank_number(uint16_t number) {
  uint16_t bnumber = (number - patch_min) / bank_size;
  bank_number = bnumber;
  bank_select_number = bnumber;
}

void Device_class::update_bank_size(uint8_t b_size) { // If the bank size changes, update the current bank number
  if (bank_size != b_size) {
    bank_size = b_size;
...
bool Device_class::flash_LEDs_for_patch_bank_switch(uint8_t sw) {
...
bool Device_class::valid_direct_select_switch(uint8_t number) {
  if (direct_select_state == 0) return ((number * 10) + (bank_select_number * 100) <= patch_max);
  else return true;
}

void Device_class::direct_select_start() {
...
uint16_t Device_class::direct_select_patch_number_to_request(uint8_t number) {
...
void Device_class::read_parameter_value_name(uint16_t number, uint16_t value, String &Output) {
  Output = "-";
}
...
bool Device_class::request_parameter(uint8_t sw, uint16_t number) {
...
uint16_t Device_class::number_of_parbank_parameters() { // By default the same as number of parameters
  return number_of_parameters();
}

uint16_t Device_class::get_parbank_parameter_id(uint16_t par_number) { // By default we want to be able to see all parameters here
  return par_number;
}

void Device_class::request_par_bank_category_name(uint8_t sw) {
  LCD_clear_SP_label(sw);
}

void Device_class::select_parameter_bank_category(uint8_t category) {
...
void Device_class::par_bank_updown(bool updown, uint8_t my_bank_size) {
  ...
  if (updown == UP) {
    ...
    DEBUGMSG("My bank size: " + String(my_bank_size));
  ...
  LCD_show_status_message("Par bank " + String(parameter_bank_number + 1) + "/" + String((number_of_parbank_parameters() - 1) / my_bank_size + 1));
...
void Device_class::request_current_assign(uint8_t sw) {
  // Not implemented in base class - move on to next switch
...
void Device_class::move_expression_pedal(uint8_t sw, uint8_t value, uint8_t exp_pedal) {}

void Device_class::toggle_expression_pedal(uint8_t sw) {
...
void Device_class::set_expr_title(uint8_t sw) {
...
bool Device_class::request_exp_pedal(uint8_t sw, uint8_t exp_pedal) {
  LCD_clear_SP_label(sw);
  return true;
}

uint8_t Device_class::read_current_device_page() { // Return the number of the current page
...
uint8_t Device_class::select_next_device_page() { // Select the next page for this device
  uint8_t tries = 4;
...
void Device_class::set_snapscene_name(uint8_t number, String &Output) {
  Output = "Not supported";
}

void Device_class::set_snapscene(uint8_t number) {}

// ********************************* Section 6: Looper control ********************************************

bool Device_class::looper_active() {
  return false;
}

uint8_t Device_class::show_looper_LED(uint8_t sw) {
  ...
  switch (SP[sw].PP_number) {
    ...
    case LOOPER_PLAY_STOP:
      if ((current_looper_state == LOOPER_STATE_PLAY) || (current_looper_state == LOOPER_STATE_PLAY_ONCE) || (current_looper_state == LOOPER_STATE_OVERDUB)) state = L_ON;
      break;
    case LOOPER_REC_OVERDUB:
      if (current_looper_state == LOOPER_STATE_RECORD) state = L_ON;
      else if (current_looper_state == LOOPER_STATE_OVERDUB) state = L_FLASHING;
      break;
  ...
  if (state == L_DIMMED) return Setting.FX_LOOPER_colour | LED_DIMMED;
  if (state == L_ON) return Setting.FX_LOOPER_colour;
  if (state == L_FLASHING) return Setting.FX_LOOPER_colour | LED_FLASHING;
  return 0;
}

void Device_class::request_looper_label(uint8_t sw) {
...
void Device_class::looper_press(uint8_t looper_cmd) {
  ...
  switch (looper_cmd) {
    ...
    case LOOPER_PLAY_STOP:
      ...
      if (current_looper_state == LOOPER_STATE_RECORD) {
        current_loop_time = current_time - looper_start_time; // Store the time
        looper_start_time = current_time;
        looper_end_time = current_time + current_loop_time; // Start showing the looper bar
      }
      if ((current_looper_state == LOOPER_STATE_STOPPED) || (current_looper_state == LOOPER_STATE_RECORD)) {
        ...
        send_looper_cmd(LOOPER_CMD_PLAY);
        looper_start_time = current_time;
        looper_end_time = current_time + current_loop_time; // Start showing the looper bar
      }
      else {
    ...
    case LOOPER_REC_OVERDUB:
      if (current_looper_state == LOOPER_STATE_STOPPED) { // If we have stopped, start a new recording
        ...
        send_looper_cmd(LOOPER_CMD_REC);
        loop_recorded = true;
        looper_overdub_happened = false;
        looper_undone = false;
        looper_start_time = current_time; // Store the time when record started
      }
      else if (current_looper_state == LOOPER_STATE_RECORD) {
        current_looper_state = LOOPER_STATE_OVERDUB;
        send_looper_cmd(LOOPER_CMD_OVERDUB);
        current_loop_time = (current_time - looper_start_time); // Store the length of the loop
        looper_start_time = current_time;
        looper_end_time = current_time + current_loop_time; // Start showing the looper bar
    ...
    case LOOPER_UNDO_REDO:
      ...
      if (looper_undone) send_looper_cmd(LOOPER_CMD_UNDO);
      else send_looper_cmd(LOOPER_CMD_REDO);
      break;
    case LOOPER_HALF_SPEED:
      ...
      if (looper_half_speed) {
        send_looper_cmd(LOOPER_CMD_HALF_SPEED);
        current_loop_time *= 2; // Double the current loop time
        if (current_looper_state != LOOPER_STATE_STOPPED)  {
          looper_end_time += (looper_end_time - current_time); // Add the remaining time to looper_end_time
          looper_start_time = looper_end_time - current_loop_time; // So the looper does not get confused on reversing on half speed
        }
      }
      else {
        send_looper_cmd(LOOPER_CMD_FULL_SPEED);
        current_loop_time /= 2; // Half the current loop time
        if (current_looper_state != LOOPER_STATE_STOPPED) {
          looper_end_time -= (looper_end_time - current_time) / 2; // Substract half the remaining time to looper_end_time
          looper_start_time = looper_end_time - current_loop_time; // So the looper does not get confused on reversing on double speed
        }
      }
      break;
    case LOOPER_REVERSE:
      ...
      if (looper_reverse) send_looper_cmd(LOOPER_CMD_REVERSE);
      else send_looper_cmd(LOOPER_CMD_FORWARD);
      if (current_looper_state != LOOPER_STATE_STOPPED) {
    ...
    case LOOPER_PLAY_ONCE:
      if (!loop_recorded) break; // Quit if no loop has been recorded yet.
      send_looper_cmd(LOOPER_CMD_PLAY_ONCE);
      if (current_looper_state == LOOPER_STATE_RECORD) current_loop_time = (current_time - looper_start_time); // Store the length of the loop
    ...
    case LOOPER_PRE_POST:
      ...
      if (looper_pre) send_looper_cmd(LOOPER_CMD_POST);
      else send_looper_cmd(LOOPER_CMD_PRE);
      break;
  }
}

void Device_class::looper_reset() {
  current_looper_state = LOOPER_STATE_STOPPED;
  send_looper_cmd(LOOPER_CMD_STOP);
  looper_end_time = 0; // Will stop the looper bar
  loop_recorded = false;
...
void Device_class::looper_timer_check() {
  if (looper_end_time > 0) {
    if (micros() > looper_end_time) { // Check if looper timer expires
      looper_start_time = looper_end_time;
      looper_end_time += current_loop_time; // Update the timer - add the loop time to it.
      if (current_looper_state == LOOPER_STATE_PLAY_ONCE) {
        current_looper_state = LOOPER_STATE_STOPPED;
        //send_looper_cmd(LOOPER_CMD_STOP); // The device will stop itself...
        looper_end_time = 0;
        update_page = REFRESH_PAGE;
      }
    }
    else if (my_looper_lcd > 0) { // Timer not expired and looper_lcd active
      // Show looper bar
      uint8_t new_bar = (looper_end_time - micros()) * 128 / current_loop_time;
      if ((looper_bar != new_bar) && (new_bar < 128)) {
        looper_bar = new_bar;
        ...
        //DEBUGMSG("Looperbar: " + String(looper_bar));
      }
    }
  }
}

void Device_class::send_looper_cmd(uint8_t cmd) {}
